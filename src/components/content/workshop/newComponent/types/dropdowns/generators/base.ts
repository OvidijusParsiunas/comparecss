...
class DropdownBase extends ComponentBuilder {
  ...
  private static addNewLayers(cardComponent: WorkshopComponent): WorkshopComponent[] {
    const layerSubcomponent = AddNewLayerComponent.add(cardComponent, LAYER_STYLES.CARD, true);
    const layerSubcomponent1 = AddNewLayerComponent.add(cardComponent, LAYER_STYLES.CARD, true);
    const layerSubcomponent2 = AddNewLayerComponent.add(cardComponent, LAYER_STYLES.CARD, true);
...
export const dropdownBase: ComponentGenerator = {
  createNewComponent(baseName?: string): WorkshopComponent {
    const dropdownMenuBaseComponent = cardBase.createNewComponent('Menu');
